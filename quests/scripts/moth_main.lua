...
require('/quests/scripts/conditions/moth_gather.lua')
require('/quests/scripts/conditions/moth_property.lua')
...
function init()
	...
	quest.setTitle(getTranslatedMiscellaneous(language, "quest_" .. config.getParameter("moth_questId", 0) .. "_title"))
	quest.setText(getTranslatedMiscellaneous(language, "quest_" .. config.getParameter("moth_questId", 0) .. "_text"))
	quest.setCompletionText(string.format(getTranslatedMiscellaneous(language, "quest_" .. config.getParameter("moth_questId", 0) .. "_completionText"), "Kyu"))
	quest.setFailureText(getTranslatedMiscellaneous(language, "quest_failureText"))
...
function buildConditions()
	...
	for _,config in pairs(conditionConfig) do
		local newCondition
		if config.type == "moth_gatherItem" then
			newCondition = buildGatherItemCondition(config)
		elseif config.type == "moth_property" then
			newCondition = buildLoveCondition(config)
		end
	...
	for _,condition in pairs(self.conditions) do
		if condition.objectiveText then
			...
			if objectiveText then
				table.insert(objectives, { getTranslatedMiscellaneous(language, condition:objectiveText()), condition:conditionMet() })
			end
		end
	...
	if conditionsMet() then
		if config.getParameter("requireTurnIn", false) then
			...
			if config.getParameter("turnInDescription") then
				quest.setObjectiveList({{string.format(getTranslatedMiscellaneous(language, "quest_turnInDescription"), "Kyu"), false}})
			end
		else
			quest.complete()
		end
